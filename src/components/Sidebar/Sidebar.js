          ...
          {links.map((item)=>{
              return (
                <div key={item.title} >
                  ...
                  {item.links.map((link)=>{
                    return (<NavLink to={`/${link.name}`} key={link.name} onClick={handleCloseSidebar} style={({isActive})=>{return isActive?{backgroundColor:currentColor}:{}}} className={({isActive})=>{
